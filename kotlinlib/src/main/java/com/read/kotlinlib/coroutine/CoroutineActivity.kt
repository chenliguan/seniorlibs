...
1.基本理解
  (1)Kotlin 协程是对线程的一种封装，同样是用来解决异步任务的方案，可以理解为一种线程框架，特点是挂起时不需要阻塞线程，更好的解决了线程切换，回调地狱的问题。
  (2)Kotlin 协程的核心竞争力在于：能简化异步并发任务，以同步的方式完成异步任务。


2.挂起函数(Suspending Function)，suspend ：挂起，暂停的意思。挂起函数的调用处称为"挂起点"。
...
//        01-14 14:59:33.125 CoroutineActivity :: I'm working2 in thread main
    ...
    fun testImage(view: View?) {
        val coroutine = TestCoroutine()
        coroutine.testImage(mIvImage)
    }
