...
public class LifeCycleTransform extends Transform {
    ...
    /**
     * 指定Transform的作用域，例如：只处理当前项目、只处理子项目等；
     *
     * Scope有7种类型：
     * 1. EXTERNAL_LIBRARIES        只有外部库
     * 2. PROJECT                   只有项目内容(当前工程的代码)
    ...
    public void transform(TransformInvocation transformInvocation) throws TransformException, InterruptedException, IOException {
        ...
        transformInputs.each { TransformInput transformInput ->
            System.out.println("find transformInput.jarInputs: " + transformInput.jarInputs.size() + " transformInput.directoryInputs: " + transformInput.directoryInputs.size())

            // 遍历jar：jar是项目依赖的三方jar
            ...
            transformInput.directoryInputs.each { DirectoryInput directoryInput ->
                System.out.println("find transformInput: " + directoryInput.name)
                File dir = directoryInput.file
                if (dir) {
                    System.out.println("find dir: " + dir.name)
